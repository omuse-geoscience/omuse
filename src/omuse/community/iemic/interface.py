...
class iemicInterface(CodeInterface, CommonCodeInterface, LiteratureReferencesMixIn):
    ...
    def __init__(self, **keyword_arguments):
        CodeInterface.__init__(
            self, name_of_the_worker="iemic_worker", **keyword_arguments
        )
    ...
    @remote_function(must_handle_array=True)
    def set_u(i=0, j=0, k=0, val=0.0):
        returns()

    @remote_function(must_handle_array=True)
    def set_v(i=0, j=0, k=0, val=0.0):
        returns()

    @remote_function(must_handle_array=True)
    def set_w(i=0, j=0, k=0, val=0.0):
        returns()

    @remote_function(must_handle_array=True)
    def set_p(i=0, j=0, k=0, val=0.0):
        returns()

    @remote_function(must_handle_array=True)
    def set_t(i=0, j=0, k=0, val=0.0):
        returns()

    @remote_function(must_handle_array=True)
    def set_s(i=0, j=0, k=0, val=0.0):
        returns()
    ...
    @remote_function(must_handle_array=True)
    def set_u_(i=0, j=0, k=0, val=0.0, sindex=0):
        returns()

    @remote_function(must_handle_array=True)
    def set_v_(i=0, j=0, k=0, val=0.0, sindex=0):
        returns()

    @remote_function(must_handle_array=True)
    def set_w_(i=0, j=0, k=0, val=0.0, sindex=0):
        returns()

    @remote_function(must_handle_array=True)
    def set_p_(i=0, j=0, k=0, val=0.0, sindex=0):
        returns()

    @remote_function(must_handle_array=True)
    def set_t_(i=0, j=0, k=0, val=0.0, sindex=0):
        returns()

    @remote_function(must_handle_array=True)
    def set_s_(i=0, j=0, k=0, val=0.0, sindex=0):
        returns()
    ...
    @remote_function(must_handle_array=True)
    def set_surface_tatm(i=0, j=0, var=0.0):
        returns()
    ...
    @remote_function(must_handle_array=True)
    def set_surface_emip(i=0, j=0, var=0.0):
        returns()
    ...
    @remote_function(must_handle_array=True)
    def set_surface_taux(i=0, j=0, var=0.0):
        returns()
    ...
    @remote_function(must_handle_array=True)
    def set_surface_tauy(i=0, j=0, var=0.0):
        returns()
    ...
    @remote_function
    def _set_double_parameter(set_name="", param_name="", value=0.0):
        returns()
...
class iemic(InCodeComponentImplementation):
    ...
    large-scale dynamical systems techniques. The term 'implicit'
    refers to this model's main design feature: all model equations
    and constraints are solved simultaneously. A Jacobian matrix and
    preconditioning techniques are available for efficient Newton
    iterations that are involved in implicit time stepping and
    continuation strategies.

    The I-EMIC contains a number of fully implicit submodels coupled
    through a modular framework. At the center of the coupled model is
    the implicit primitive equation ocean model THCM [1]. THCM is
    extended with models of modest complexity that describe varying
    aspects of the climate, including atmospheric heat and moisture
    transport, the formation of sea ice and the adjustment of albedo
    over snow and ice.
    ...
    def define_state(self, handler):
        ...
        handler.add_transition(
            "INITIALIZED", "OCEAN-PARAM-CONTINUATION-NOPARAM", "commit_parameters"
        )
        ...
        handler.add_transition(
            "INITIALIZED", "INITIALIZED", "set_continuation_transition"
        )
        ...
        handler.add_method("OCEAN-PARAM-CONTINUATION-PARAM", "step_continuation")
    ...
    def define_grids(self, handler):
        ...
        handler.define_grid(
            "v_grid", axes_names=["lon", "lat", "z"], grid_class=StructuredGrid
        )
        ...
        handler.define_grid(
            "surface_t_grid", axes_names=["lon", "lat"], grid_class=CartesianGrid
        )
    ...
    def _generate_parameter_setter_getters(
        self, paramSet, sublist, paramName, paramType
    ):
        # define getter (closure or partial..)
        def getter():
            return getattr(self, "_get_" + paramType + "_parameter")(
                paramSet, "->".join(sublist + [paramName])
            )

        # define setter
        def setter(val):
            return getattr(self, "set_" + paramType + "_parameter")(
                paramSet, "->".join(sublist + [paramName]), val
            )
    ...
    def define_parameter_set(self, handler, paramSet, sublist=[]):
        ...
        for j in range(0, paramCount):
            ...
            paramType = self._get_parameter_type(
                paramSet, "->".join(sublist + [paramName])
            )
            if paramType == "ParameterList":
                ...
                longname = (
                    longname.replace(" ", "_").replace("-", "_").replace("/", "_")
                )
                parameter_set_name = "__".join([paramSet] + sublist)

                getter, setter = self._generate_parameter_setter_getters(
                    paramSet, sublist, paramName, paramType
                )
                ...
                # get default
                default = getattr(self, "_get_default_" + paramType + "_parameter")(
                    paramSet, "->".join(sublist + [paramName])
                )
                # define parameter
    ...
    def define_parameters(self, handler):
        ...
        handler.add_interface_parameter(
            "timestepping_theta", "timestepping theta parameter [0-1]", 1.0
        )
        handler.add_interface_parameter(
            "timestepping_dt", "timestep for timestepper", 1.0
        )
    ...
    def solve(self, *arg):
        if len(arg) == 2:
            ...
            if jac is not None:
                raise Exception(
                    "solve only supports internal interface jacobian with jac=None"
                )
        elif len(arg) == 1:
            state = arg[0]
        else:
    ...
    def get_default_parameter(self, name):
        type = self.get_parameter_type(name)
        set_name, param_name = name.split("->", 1)
        return getattr(self, "_get_default_" + type + "_parameter")(
            set_name, param_name
        )
    ...
    def set_double_parameter(self, set_name="", param_name="", value=0.0):
