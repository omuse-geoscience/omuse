...
def temppot(S,T,P=0. | units.dbar ,Pref=0. | units.dbar):
    ...
    return _seawater.temppot( _S,_T,_P,_Pref) | units.C
...
def temppot0(S,T,P=0. | units.dbar):
    ...
    _T=T.value_in(units.C)
    _P=P.value_in(units.dbar)
    return _seawater.temppot0( _S,_T,_P) | units.C

def freezept(S,P=0. | units.dbar):
    """ Freezing point """
    _S=S.value_in(units.psu)
    _P=P.value_in(units.dbar)
    return _seawater.freezept(_S,_P) | units.C
...
def EOS_linear(S, T, rho0=1028. | units.kg/units.m**3, T0=10. | units.C, S0=35. | units.psu, alpha=0.00017 | units.C**-1, beta=0.00076 | units.psu**-1):
    """
